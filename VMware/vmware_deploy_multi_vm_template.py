...
# Deploy multiple VMs from the template
vm_template_deploy_service = vm_template_deploy.TemplateDeploy(client)
deploy_spec = vm_template_deploy_service.create_spec(name=VM_NAME_PREFIX,
                                          folder=FOLDER_ID, 
...
for i in range(NUM_VMS):
    # Update the name of the VM for each iteration
    deploy_spec.name = f"{VM_NAME_PREFIX}-{i+1}"
